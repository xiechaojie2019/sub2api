...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			createdAt,
			nullString(log.RequestBody),  // request_body
			nullString(log.ResponseBody), // response_body
			usageBodyTruncatedArg(log.RequestBody, log.RequestBodyTruncated),   // request_body_truncated
			usageBodyTruncatedArg(log.ResponseBody, log.ResponseBodyTruncated), // response_body_truncated
		},
	}
}

// usageBodyTruncatedArg 仅在捕获了请求/响应体时写入截断标记；未捕获（body 为 nil）时为 NULL，
// 避免历史行语义（NULL = 未捕获）与"捕获了但未截断"混淆。
func usageBodyTruncatedArg(body *string, truncated bool) sql.NullBool {
	if body == nil {
		return sql.NullBool{}
	}
	return sql.NullBool{Bool: truncated, Valid: true}
}
