...
func RegisterGatewayRoutes(
	...
	// 用量请求/响应体捕获（系统开关控制，默认关闭）。注册在认证之后：
	// 未认证请求不值得付出读取/缓冲代价。
	usageBodyCapture := handler.UsageBodyCaptureMiddleware(settingService)
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), usageBodyCapture, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), usageBodyCapture, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
	...
	antigravityV1.Use(usageBodyCapture)
	...
	antigravityV1Beta.Use(usageBodyCapture)
